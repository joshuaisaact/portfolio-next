...
export const postMetadata = {
  title: "Building a Live London Underground Tracker: Learning Go",
  date: "2024-01-15",
  featured_image: "/images/blog/tfl-pulse-mvp.webp",
  excerpt:
...
This introduced me to the fundamentals of the language at a pretty rapid clip. But as a kinesthetic learner, I next needed an actual project to sink my metaphorical teeth into, to solidify this learning and build on it. No more tutorials, no more watching videos - just pick something interesting and figure it out.
...
For the MVP, I needed to strip this down to just the essential fields. In Go, this meant defining a clean struct to receive just what we need:
...
TfL provides train locations as text descriptions like "At Victoria" or "Between Kings Cross and Euston". I needed to map these to something more structured. Go's iota gives us a clean way to define enumerated states, but we also need to handle JSON serialization:
...
Most importantly, it showed how Go's type system can help transform complex API responses into clean, usable data structures perfect for our needs.
...
The next big challenge is proper position interpolation. Right now I'm basically just reporting what TfL tells me, but real trains don't teleport between stations. I'm looking at:

- Building a proper line graph
- Interpolating positions between stations
- Adding historical data storage (probably TimescaleDB)
- Making the visualization actually useful

But the biggest lesson? The best way to learn a new language is to build something real. Videos and tutorials are great, but nothing beats struggling through actual problems and writing actual code.

\_Next up: Building a proper graph representation of the Victoria Line, because trains should probably follow the tracks...

...
It was unmaintainable, unreliable, and worst of all - the AI would forget what it was supposed to do halfway through conversations. Classic rookie mistake.
...
4. **Agent Frameworks** - Pre-built team coordination. CrewAI, AutoGen, LangGraph. Agents that delegate to other agents. Powerful but sometimes overkill.

The approach I landed on - orchestrated specialists - emerged from necessity. I needed something more maintainable than a monolith but didn't need the complexity of full agent frameworks. It turns out this middle ground is exactly right for many real-world applications.
...
By using structured output schemas (thank you Gemini!), the intent agent reliably categorizes requests and extracts parameters. No more parsing free-text responses or dealing with hallucinated JSON.
...
*Maya is live at [yoga.joshdesk.live](https://yoga.joshdesk.live). Built with Bun, Elysia, Google's Gemini, and a lot of careful orchestration.*